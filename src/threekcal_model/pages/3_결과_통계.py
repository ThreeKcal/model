...
try:
    ...
    if lnum != pnum:
        ...
        tdf.loc[(tdf['label'].notnull()) & (~tdf['prediction_result'].isin(df['label'])), 'prediction_result'] = tdf[
            (tdf['label'].notnull()) & (~tdf['prediction_result'].isin(df['label']))]['prediction_result'].apply(
            lambda x: random.choice(uq_label_list[uq_label_list != x]))
